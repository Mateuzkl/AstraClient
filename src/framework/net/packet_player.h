...
public:
    ...
    void start(std::function<void(std::shared_ptr<std::vector<uint8_t>>)> recvCallback, std::function<void(asio::error_code)> disconnectCallback);
    void stop();
...
private:
    ...
    std::function<void(asio::error_code)> m_disconnectCallback;
};
