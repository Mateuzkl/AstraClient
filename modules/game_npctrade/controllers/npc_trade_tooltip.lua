...
local function safeThingTypeMarketData(thingType)
  local data = safeCallMethod(thingType, "getMarketData")
...
local function safeThingTypeName(thingType)
  local data = safeThingTypeMarketData(thingType)
...
local function safeThingTypeDescription(thingType)
  local data = safeThingTypeMarketData(thingType)
  if data and type(data.description) == "string" then
...
local function safeItemName(item)
  local value = safeCallMethod(item, "getName")
...
local function safeItemDescription(item)
  local value = safeCallMethod(item, "getDescription")
  if type(value) == "string" then
    return value
  end
  return ""
...
local function safeThingTypeFlag(thingType, methodName)
  return safeCallMethod(thingType, methodName) == true
end

local function resolveDisplayName(displayName, item, thingType)
  if type(displayName) == "string" and displayName ~= "" then
    return displayName
  end

  local itemName = safeItemName(item)
  if itemName ~= "" then
    return itemName
  end

  return safeThingTypeName(thingType)
end

local function resolveDescription(description, item, thingType)
  if type(description) == "string" and description ~= "" then
    return description
  end

  local itemDescription = safeItemDescription(item)
  if itemDescription ~= "" then
    return itemDescription
  end

  return safeThingTypeDescription(thingType)
end
...
local function toAscii(s)
  ...
  local r = s
  for _, p in ipairs(ASCII_REPLACEMENTS) do
    r = r:gsub(p[1], p[2])
  end
...
local function classify(item, thingType, displayName, itemId)
  ...
  local typeName = safeThingTypeName(thingType)
  ...
  local isFluid = safeThingTypeFlag(thingType, "isFluidContainer")
  local isSplash = safeThingTypeFlag(thingType, "isSplash")
  local isChargeable = safeThingTypeFlag(thingType, "isChargeable")
  local isStackable = safeThingTypeFlag(thingType, "isStackable")

  if isFluid and not isSplash then
    return "potion"
  end

  if isChargeable and isStackable and not isFluid then
    return "rune"
  end
...
local function fillAndShow(hoverData)
  ...
  local resolvedName = resolveDisplayName(displayName, item, tt)
  local typeName = safeThingTypeName(tt)
  local kind = classify(item, tt, resolvedName, id)
  if not kind then
    return false
  end
  ...
  local dispName = resolvedName
  dispName = toAscii(dispName)
  local desc = resolveDescription(description, item, tt)
  ...
  if kind == "exp_boost" then
    ...
    if kind == "rune" then
      ...
      if CUSTOM_POTION_IDS[id] then
        ...
        local nameForSpell = resolvedName ~= "" and resolvedName or typeName
        local canaryP = resolveCanaryFlaskPotion(id, typeName, resolvedName)
        if canaryP then
          ...
          if not spellPotion and typeName ~= "" then
            spellPotion = findInstantSpellForPotionName(typeName:lower())
          end
          if spellPotion then
...
function NpcTradeTooltip.init()
  ...
  root = g_ui.loadUI("/game_npctrade/npc_trade_tooltip", g_ui.getRootWidget())
  if not root then
...
if modules and modules.game_npctrade then
  modules.game_npctrade.NpcTradeTooltip = NpcTradeTooltip
end
