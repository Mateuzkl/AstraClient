...
function onOpenNpcTrade(items, currencyId, currencyName)
    ...
    if controllerNpcTrader.syncNpcDialogLayout then
        controllerNpcTrader:syncNpcDialogLayout()
    end
    ...
    if isNewSession then
        ...
        if controllerNpcTrader.tradeMode == controllerNpcTrader.BUY and #controllerNpcTrader.buyItems == 0 then
            controllerNpcTrader.tradeMode = initialMode
        elseif controllerNpcTrader.tradeMode == controllerNpcTrader.SELL and #controllerNpcTrader.sellItems == 0 then
            controllerNpcTrader.tradeMode = initialMode
        end
...
function controllerNpcTrader:setupTradeControlHooks()
    if not self.ui or self.ui:isDestroyed() then
        return
    end

    if self.setupTradeListScrollBar then
        self:setupTradeListScrollBar()
    end

    local buyTab = self.ui:recursiveGetChildById("tabBuy")
    if buyTab and not buyTab:isDestroyed() then
        buyTab.onClick = function()
            controllerNpcTrader:setTradeMode(controllerNpcTrader.BUY)
            return true
        end
    end

    local sellTab = self.ui:recursiveGetChildById("tabSell")
    if sellTab and not sellTab:isDestroyed() then
        sellTab.onClick = function()
            controllerNpcTrader:setTradeMode(controllerNpcTrader.SELL)
            return true
        end
    end

    local tradeButton = self.ui:recursiveGetChildById("toggleButton")
    if tradeButton and not tradeButton:isDestroyed() then
        tradeButton.onClick = function()
            controllerNpcTrader:executeTrade()
            return true
        end
    end

    local amountScroll = self.ui:recursiveGetChildById("amountScrollBar")
    if amountScroll and not amountScroll:isDestroyed() then
        amountScroll.onValueChange = function(_, value)
            controllerNpcTrader:onAmountScrollBarChange(value)
        end
    end

    local searchInput = self.ui:recursiveGetChildById("tradeSearchInput")
    if searchInput and not searchInput:isDestroyed() then
        searchInput.onMousePress = function(widget)
            if controllerNpcTrader.focusNpcTextInput then
                return controllerNpcTrader:focusNpcTextInput(widget)
            end
            return false
        end
        searchInput.onFocusChange = function(widget, focused)
            if focused then
                if controllerNpcTrader.focusNpcTextInput then
                    controllerNpcTrader:focusNpcTextInput(widget)
                end
            elseif controllerNpcTrader.releaseNpcTextInput then
                controllerNpcTrader:releaseNpcTextInput(widget)
            end
        end
        searchInput.onTextChange = function(widget)
            controllerNpcTrader:filterTradeList((widget and widget.getText and widget:getText()) or "")
        end
    end
    ...
    local searchClear = self:findWidget(".tradeSearchClear")
    if searchClear and not searchClear:isDestroyed() then
        searchClear.onClick = function()
            controllerNpcTrader:clearSearch()
            return true
        end
    end
end

function controllerNpcTrader:getTradeListWidgets()
    if not self.ui or self.ui:isDestroyed() then
        return nil, nil
    end

    local list = self.ui:recursiveGetChildById("tradeListScroll")
    local scrollBar = self.ui:recursiveGetChildById("tradeListScrollbar")
    return list, scrollBar
end

function controllerNpcTrader:setupTradeListScrollBar(resetValue)
    local list, scrollBar = self:getTradeListWidgets()
    if not list or list:isDestroyed() or not scrollBar or scrollBar:isDestroyed() then
        return
    end

    if list.setVerticalScrollBar and list._npcTradeScrollBar ~= scrollBar then
        list:setVerticalScrollBar(scrollBar)
        list._npcTradeScrollBar = scrollBar
    end

    if scrollBar.setStep then
        scrollBar:setStep(controllerNpcTrader.ITEM_ROW_HEIGHT)
    end
    scrollBar.pixelsScroll = true

    list.onScrollChange = function(widget, offset)
        controllerNpcTrader:onTradeScroll(widget, offset)
    end

    if resetValue and scrollBar.setValue then
        scrollBar:setValue(0)
    end
    if resetValue and list.setVirtualOffset then
...
function controllerNpcTrader:resetTradeListRender()
    if resetHtmlFor then
        resetHtmlFor(self, "tradeListScroll")
    else
        local list = self.ui and self.ui:recursiveGetChildById("tradeListScroll")
        if list and not list:isDestroyed() and list.destroyChildren then
            list:destroyChildren()
        end
    end

    self:setupTradeListScrollBar(true)
end
...
function controllerNpcTrader:loadNextBatch()
    ...
    if refreshHtmlFor then
        refreshHtmlFor(self, "tradeListScroll")
    end
...
function controllerNpcTrader:onTradeScroll(widget, offset)
    if not self.currentList or self.loadedItems >= #self.currentList then
        return
    end
    ...
    local value = (offset and offset.y) or 0
...
function controllerNpcTrader:onTradeListRendered()
    ...
    if list then
            ...
            if item then
                ...
                if not child._npcTradeTooltipHoverBound then
                    ...
                    child.onHoverChange = function(_, hovered)
                        child._npcTradeHovered = hovered
                        applyTradeListSelectionStyle(child, child.tradeItem == self.selectedItem, child._npcTradeRowIndex)
                        self:onNpcTradeRowHover(item, hovered)
                    end
                end
                local icon = child:getChildByIndex(1)
                if icon and not icon:isDestroyed() then
                    icon.tradeItem = item
                    icon.onMouseRelease = function(widget, mousePos, mouseButton)
                        self:onTradeItemMouseRelease(item, widget, mousePos, mouseButton)
                    end
                    if not icon._npcTradeTooltipHoverBound then
                        icon._npcTradeTooltipHoverBound = true
                        icon.onHoverChange = function(_, hovered)
                            child._npcTradeHovered = hovered
                            applyTradeListSelectionStyle(child, child.tradeItem == self.selectedItem, child._npcTradeRowIndex)
                            self:onNpcTradeRowHover(item, hovered)
                        end
        ...
        self:syncTradeListSelection()
    end
end

local function resolveTradeItemFromWidget(item, widget)
    if item then
        return item
    end
    ...
    while current do
        if current.tradeItem then
            return current.tradeItem
        end
...
applyTradeListSelectionStyle = function(rowWidget, selected, rowIndex)
    if not rowWidget or (rowWidget.isDestroyed and rowWidget:isDestroyed()) then
        return
    end
    ...
    if rowWidget._npcTradeHovered then
        background = "#505050"
    end
    if selected then
        background = "#d90000"
    end
    if rowWidget.setBackgroundColor then
        rowWidget:setBackgroundColor(background)
    end
    if rowWidget.setBorderWidth then
        rowWidget:setBorderWidth(1)
    end
    if rowWidget.setBorderColor then
        rowWidget:setBorderColor(selected and "#ff1a1a" or background)
    end
end

local function addPopupCheck(menu, label, checked, callback)
    if menu.addCheckBox then
        return menu:addCheckBox(label, checked, function()
            callback()
        end)
    end

    if menu.addCheckBoxOption then
        return menu:addCheckBoxOption(label, function()
            callback()
        end, nil, checked)
    end

    local prefix = checked and "[x] " or "[ ] "
    return menu:addOption(prefix .. label, callback)
end

function controllerNpcTrader:syncTradeListSelection()
    local list = self:findWidget("#tradeListScroll")
    if not list then
        return
    end
    for i = 1, list:getChildCount() do
        local child = list:getChildByIndex(i)
        if child and child.tradeItem then
            child._npcTradeRowIndex = i
            applyTradeListSelectionStyle(child, child.tradeItem == self.selectedItem, i)
        end
    end
end

function controllerNpcTrader:onTradeItemMouseRelease(item, widget, mousePos, mouseButton)
    item = resolveTradeItemFromWidget(item, widget)
    if not item or not item.ptr then
        return false
    end
    ...
    if mouseButton == MouseRightButton then
        ...
        for _, opt in ipairs(sortOptions) do
            addPopupCheck(menu, tr(opt.label), self.sortBy == opt.key, function()
        ...
        if self.tradeMode == controllerNpcTrader.BUY then
            if self.currencyId == controllerNpcTrader.DEFAULT_CURRENCY_ID then
                addPopupCheck(menu, tr("Buy in shopping bags"), self.buyWithBackpack, function()
                    self:toggleBuyWithBackpack()
                end)
            end
            addPopupCheck(menu, tr("Ignore capacity"), self.ignoreCapacity, function()
                self:toggleIgnoreCapacity()
            end)
        else
            addPopupCheck(menu, tr("Sell equipped"), not self.ignoreEquipped, function()
        ...
        addPopupCheck(menu, tr("Show search field"), self.showSearchField, function()
            self:toggleSearchField()
        end)
        addPopupCheck(menu, tr("Do not show a warning when trading large amounts"), self.noLargeAmountWarning, function()
...
function controllerNpcTrader:refreshFooter()
    ...
    local totalPriceDisplay = footer:recursiveGetChildById("totalPriceDisplay")
    if totalPriceDisplay and totalPriceDisplay.setText then
        totalPriceDisplay:setText(self:getTotalPriceText())
    end

    local playerGoldDisplay = footer:recursiveGetChildById("playerGoldDisplay")
    if playerGoldDisplay and playerGoldDisplay.setText then
        playerGoldDisplay:setText(self:getPlayerGoldText())
    end

    local playerCurrencyDisplay = footer:recursiveGetChildById("playerCurrencyDisplay")
    if playerCurrencyDisplay and playerCurrencyDisplay.setText then
        playerCurrencyDisplay:setText(self:getPlayerCurrencyText())
    end
...
function controllerNpcTrader:setupTradeAmountInputHooks()
    ...
    input.onClick = function(widget)
        ...
        if controllerNpcTrader and controllerNpcTrader.onTradeAmountInputFocus then
            controllerNpcTrader:onTradeAmountInputFocus(widget)
        end
        return false
    end
    input.onTextChange = function(widget, value)
        if controllerNpcTrader and controllerNpcTrader.onAmountInputChange then
            controllerNpcTrader:onAmountInputChange({
                target = widget,
                value = value
            })
        end
    ...
    input.onFocusChange = function(widget, focused)
        ...
        if controllerNpcTrader and controllerNpcTrader.releaseNpcTextInput then
            controllerNpcTrader:releaseNpcTextInput(widget)
        end
    end
end
...
function controllerNpcTrader:getPlayerMoney()
    ...
    if cid == controllerNpcTrader.DEFAULT_CURRENCY_ID then
        return self:getAstraGoldBalance()
    end
    return self:getAstraCustomCurrencyBalance()
end
...
function controllerNpcTrader:onNpcTradeRowHover(item, hovered)
    local nt = modules.game_npctrade and modules.game_npctrade.NpcTradeTooltip
