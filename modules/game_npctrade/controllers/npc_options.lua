local function addMenuCheck(menu, label, checked, callback)
...
function controllerNpcTrader:onOptionsClick()
    ...
    if self.tradeMode == controllerNpcTrader.BUY then
        addMenuCheck(menu, "Ignore Capacity", self.ignoreCapacity, function()
            self:toggleIgnoreCapacity()
        end)
        if self.currencyId == controllerNpcTrader.DEFAULT_CURRENCY_ID then
            addMenuCheck(menu, "Buy with Backpack", self.buyWithBackpack, function()
                self:toggleBuyWithBackpack()
            end)
        end
    else
        addMenuCheck(menu, "Sell Equipped", not self.ignoreEquipped, function()
