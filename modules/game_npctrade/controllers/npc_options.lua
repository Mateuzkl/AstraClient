function controllerNpcTrader:onOptionsClick()
    ...
    if self.tradeMode == controllerNpcTrader.BUY then
        menu:addCheckBoxOption("Ignore Capacity", function()
            self:toggleIgnoreCapacity()
        end, "", self.ignoreCapacity)
        if self.currencyId == controllerNpcTrader.DEFAULT_CURRENCY_ID then
            menu:addCheckBoxOption("Buy with Backpack", function()
                self:toggleBuyWithBackpack()
            end, "", self.buyWithBackpack)
        end
    else
        menu:addCheckBoxOption("Sell Equipped", function()
            self:toggleIgnoreEquipped()
        end, "", not self.ignoreEquipped)
    end
    menu:display()
end
