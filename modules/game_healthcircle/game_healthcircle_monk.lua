...
local function hasMonkFeature()
    return GameVocationMonk and g_game.getFeature and g_game.getFeature(GameVocationMonk)
end
...
function switchToMonkMode(enabled)
    ...
    if enabled then
        ...
        if player then
            monkHarmony = player.getHarmony and clampMonkHarmony(player:getHarmony()) or 0
            monkSerene = player.isSerene and player:isSerene() or false
        else
            monkHarmony = 0
            monkSerene = false
        end
...
function checkMonkVocation()
    ...
    local isMonk = hasMonkFeature() and player.isMonk and player:isMonk()
