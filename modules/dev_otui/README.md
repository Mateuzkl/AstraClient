...
Originally created by **Renan Miyashiro (`renanale`)** in
[opentibiabr/otclient#1782](https://github.com/opentibiabr/otclient/pull/1782).
This AstraClient port preserves that attribution while adapting resource paths,
module lifecycle and project-file safety to Astra's architecture.
...
(every one under `/data/styles`) declares only styles and has no main widget, so
...
Style definitions that contain child widgets are intentionally editable. Astra's
`UIManager` expands those non-unique child nodes when the style is applied, so
rejecting them would remove a valid engine feature. Insert and delete still
re-read the file and compare the selected style, id and ancestry before writing;
the self-test includes a style with children plus a file-reordering refusal.
...
When the client is running only from `data.zip`, embedded memory or another
read-only package, preview remains available and every write action is blocked
...
It asserts, among others: byte-for-byte round-trip, comments and blank lines,
composite blocks such as `layout:` not being mistaken for child widgets,
anchors, edits not touching neighbouring lines, insertion indentation, and
removal of a block taking its children along. It also covers reordered and
renamed widgets, unresolved references, safe paths for all three project roots,
read-only mode, backup/write/read-back failures, restoration of the original,
and confirmation that every refused flow leaves the target bytes unchanged.
