...
local function extractFileNameNoExt(filePath)
    local name = tostring(filePath or "")
    name = name:gsub("^.*[\\/]", "")
    name = name:gsub("%.[^.]+$", "")
    return name
end

local function sanitizeImportedCavebotName(name)
    local sanitized = tostring(name or ""):gsub("^%s+", ""):gsub("%s+$", "")
    sanitized = sanitized:gsub("%s+", "_")
    sanitized = sanitized:gsub("[^%w_%-]", "")
    if sanitized == "" then
        sanitized = "imported_zerobot_cavebot"
    end
    return sanitized
end

local function mapZeroBotWaypointType(zbType)
    local wpType = tonumber(zbType) or 1
    if wpType == 0 then
        return "stand"
    elseif wpType == 1 then
        return "node"
    elseif wpType == 4 or wpType == 5 or wpType == 15 then
        return "rope"
    elseif wpType == 6 or wpType == 16 then
        return "hole"
    elseif wpType == 7 or wpType == 8 or wpType == 17 then
        return "use"
    elseif wpType == 9 then
        return "label"
    elseif wpType == 10 then
        return "goto"
    elseif wpType == 14 then
        return "door"
    end
    return "node"
end

local function isZeroBotLureMarkerType(zbType)
    local wpType = tonumber(zbType) or -1
    return wpType == 2 or wpType == 3 or wpType == 12 or wpType == 13
end

local function defaultCavebotConfig()
    return {
        nodeDistance = 2,
        stopToKillDistance = 2,
        creaturesToStop = 8,
...
local function addSpecialAreaIfMissing(specialAreas, pos)
    if not pos then
        return
    end
    ...
    table.insert(specialAreas, { id = #specialAreas + 1, position = pos, type = 90 })
end

local function convertZeroBotCavebotToLocal(zbData, cavebotName)
    local converted = {
        name = cavebotName,
        waypoints = {},
        specialAreas = {},
        config = defaultCavebotConfig(),
        supplies = {},
        supplySettings = {}
    }

    local zbWaypoints = type(zbData.waypoints) == "table" and zbData.waypoints or {}
    for _, zbWp in ipairs(zbWaypoints) do
        local x = tonumber(zbWp and zbWp.x)
        local y = tonumber(zbWp and zbWp.y)
        local z = tonumber(zbWp and zbWp.z)
        if x and y and z then
            local wpType = mapZeroBotWaypointType(zbWp.waypointType)
            local localWp = {
                index = #converted.waypoints + 1,
                position = { x = math.floor(x), y = math.floor(y), z = math.floor(z) },
                type = wpType
            }

            local dataStr = tostring(zbWp.data or "")
            if wpType == "label" then
                localWp.label = (dataStr ~= "" and dataStr) or ("label_" .. tostring(localWp.index))
            elseif wpType == "goto" then
                localWp.label = dataStr
                localWp.gotoCondition = "none"
                localWp.gotoStamina = 0
            end

            table.insert(converted.waypoints, localWp)

            if isZeroBotLureMarkerType(zbWp.waypointType) then
                addSpecialAreaIfMissing(converted.specialAreas, localWp.position)
            end
        end
    end

    if type(zbData.specialAreas) == "table" then
        for _, area in ipairs(zbData.specialAreas) do
            local pos = nil
            if type(area) == "table" then
                if type(area.position) == "table" then
                    local x = tonumber(area.position.x)
                    local y = tonumber(area.position.y)
                    local z = tonumber(area.position.z)
                    if x and y and z then
                        pos = { x = math.floor(x), y = math.floor(y), z = math.floor(z) }
                    end
                else
                    local x = tonumber(area.x)
                    local y = tonumber(area.y)
                    local z = tonumber(area.z)
                    if x and y and z then
                        pos = { x = math.floor(x), y = math.floor(y), z = math.floor(z) }
                    end
                end
            end
            addSpecialAreaIfMissing(converted.specialAreas, pos)
        end
    end

    hunting_recorderModule.normalizeCavebotData(converted)
    return converted
end

function hunting_recorderModule.importZeroBotCavebot()
    local useAsync = g_platform and type(g_platform.showOpenFileDialogAsync) == "function"
    if not useAsync and (not g_platform or type(g_platform.showOpenFileDialog) ~= "function") then
        modules.game_textmessage.displayFailureMessage("Import .cb indisponivel nesta plataforma.")
        return
    end

    local function processImportFile(filePath)
        if not filePath or filePath:len() == 0 then
            return
        end

        local fileContents = nil
    local isAbsolute = filePath:match("^[A-Za-z]:[/\\]") or filePath:match("^[/\\]")
    if isAbsolute then
        local file = io.open(filePath, "r")
        if not file then
            modules.game_textmessage.displayFailureMessage("Could not open file: " .. tostring(filePath))
            return
        end
        ...
        if not g_resources.fileExists(filePath) then
            modules.game_textmessage.displayFailureMessage("File not found: " .. tostring(filePath))
            return
        end
        fileContents = g_resources.readFileContents(filePath)
    end

    if not fileContents or fileContents:gsub("%s", "") == "" then
        modules.game_textmessage.displayFailureMessage("Arquivo vazio.")
        return
    end

    local okDecode, zbData = pcall(function() return json.decode(fileContents) end)
    if not okDecode or type(zbData) ~= "table" then
        modules.game_textmessage.displayFailureMessage("JSON invalido no arquivo .cb.")
        return
    end
    if type(zbData.waypoints) ~= "table" or #zbData.waypoints == 0 then
        modules.game_textmessage.displayFailureMessage("ZeroBot cavebot has no waypoints.")
        return
    end

    local suggestedName = sanitizeImportedCavebotName(extractFileNameNoExt(filePath))

    local inputBox
    local function doImportWithName(rawName)
        local cavebotName = sanitizeImportedCavebotName(rawName)
        local converted = convertZeroBotCavebotToLocal(zbData, cavebotName)

        local function persistImport()
            local saved = saveCavebotToFile(cavebotName, converted, true)
            if not saved then
                modules.game_textmessage.displayFailureMessage("Failed to save imported cavebot: " .. cavebotName)
                return
            end
            selectedCavebotName = cavebotName
            hunting_recorderModule.refreshCavebotsList()
            hunting_recorderModule.refreshMainCavebotsList()
            hunting_recorderModule.loadCavebotByName(cavebotName)
            modules.game_textmessage.displayGameMessage(
                string.format("Importado ZeroBot .cb: %s (%d waypoints)", cavebotName, #converted.waypoints)
            )
        end

        local existingFile = "/helper/cavebots/" .. cavebotName .. ".json"
        if g_resources.fileExists(existingFile) then
            local messageBox
    ...
    inputBox = HelperUIInputBox.create("Import ZeroBot Cavebot", okCallback, cancelCallback)
    inputBox:addLineEdit("Cavebot name:", suggestedName)
    inputBox:display()
    end -- processImportFile

    if useAsync then
        g_platform.showOpenFileDialogAsync(
            tr("Select ZeroBot cavebot (.cb)"),
            tr("ZeroBot Cavebot (*.cb)|*.cb|JSON Files (*.json)|*.json|All Files (*.*)|*.*"),
            processImportFile
        )
    else
        local filePath = g_platform.showOpenFileDialog(
            tr("Select ZeroBot cavebot (.cb)"),
            tr("ZeroBot Cavebot (*.cb)|*.cb|JSON Files (*.json)|*.json|All Files (*.*)|*.*")
        )
