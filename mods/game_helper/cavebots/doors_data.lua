-- CaveBot Doors Data
-- Copia das tabelas de portas do servidor (data/libs/tables/doors.lua),
-- usada pela action "door" do cavebot para decidir entre USAR (abrir) e ANDAR (atravessar).
--
-- Semantica:
--   lockedDoor -> porta trancada ("It is locked"), nao passa sem chave  -> bloqueia
--   closedDoor -> porta fechada normal, abre ao usar                    -> da USE
--   openDoor   -> porta aberta, tile andavel                            -> ANDA

CavebotDoors = CavebotDoors or {}

-- ID das chaves (21392 e so da quest de dawnport)
local keysID = { 2967, 2968, 2969, 2970, 2971, 2972, 2973, 21392 }
...
local CustomDoorTable = {
  ...
  { closedDoor = 61762, openDoor = 61763 },
  { closedDoor = 61764, openDoor = 61765 },
  { closedDoor = 61766, openDoor = 61767 },
  { closedDoor = 61768, openDoor = 61769 },
  { closedDoor = 61770, openDoor = 61771 },
  { closedDoor = 61772, openDoor = 61773 },
  { closedDoor = 61774, openDoor = 61775 },
  { closedDoor = 61776, openDoor = 61777 },
  { closedDoor = 61777, openDoor = 61778 },
  -- Summer Update 2025
  { closedDoor = 51431, openDoor = 51432 },
  { closedDoor = 51433, openDoor = 51434 },
  { closedDoor = 51435, openDoor = 51437 },
  { closedDoor = 51436, openDoor = 51438 },
  { closedDoor = 50535, openDoor = 50539 },
  { closedDoor = 50537, openDoor = 50540 },
  { closedDoor = 50524, openDoor = 50528 },
  { closedDoor = 50526, openDoor = 50529 },
  -- Winter Update 2025
  { closedDoor = 52183, openDoor = 52187 },
  { closedDoor = 52185, openDoor = 52188 },
  { closedDoor = 52254, openDoor = 52258 },
  { closedDoor = 52256, openDoor = 52259 },
}

local QuestDoorTable = {
...
-- ============================================================================
-- Lookup sets (pre-computados no load)
-- ============================================================================

local closedToOpen = {}  -- closedDoor id -> openDoor id (porta que ABRE ao usar)
local openSet = {}       -- openDoor id -> true (porta ja aberta, andavel)
local lockedSet = {}     -- lockedDoor id -> true (trancada, nao passa sem chave)

local function registerPair(closedId, openId)
  if closedId then
    closedToOpen[closedId] = openId or true
  end
  if openId then
    openSet[openId] = true
  end
...
for _, d in ipairs(KeyDoorTable) do
  if d.lockedDoor then lockedSet[d.lockedDoor] = true end
  registerPair(d.closedDoor, d.openDoor)
end
for _, d in ipairs(CustomDoorTable) do registerPair(d.closedDoor, d.openDoor) end
for _, d in ipairs(QuestDoorTable) do registerPair(d.closedDoor, d.openDoor) end
for _, d in ipairs(LevelDoorTable) do registerPair(d.closedDoor, d.openDoor) end

-- ============================================================================
-- API publica
-- ============================================================================

-- Porta aberta (andavel) -> deve ANDAR atravessando.
function CavebotDoors.isOpen(id)
  return id ~= nil and openSet[id] == true
end

-- Porta fechada normal -> deve USAR para abrir. Retorna tambem o id esperado aberto.
function CavebotDoors.isClosed(id)
  if id == nil then return false end
  local openId = closedToOpen[id]
  if openId == nil then return false end
  return true, (openId ~= true) and openId or nil
end

-- Porta trancada ("It is locked") -> nao passa sem chave.
function CavebotDoors.isLocked(id)
  return id ~= nil and lockedSet[id] == true
end

-- Verdadeiro para qualquer id que pertenca a uma porta conhecida (qualquer estado).
function CavebotDoors.isKnownDoor(id)
  if id == nil then return false end
  return openSet[id] == true or closedToOpen[id] ~= nil or lockedSet[id] == true
end

CavebotDoors.keysID = keysID
